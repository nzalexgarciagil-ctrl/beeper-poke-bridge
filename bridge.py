...
(WhatsApp, Discord, Telegram, Instagram, etc.) and runs a two-stage pipeline:
  ...
  1. Gatekeeper (gatekeeper.py): a fast LLM triage call -- modelled on Poke's
...
# Load secrets from .env before importing gatekeeper (which reads creds at import).
...
async def send_batch_to_poke(chat_id: str, chat_info: dict, entries: list[dict]):
    ...
    event_text = gatekeeper.render_event(
        payload.get("platform", "Unknown"), payload.get("chat", {}), sender, msgs, payload.get("context"),
    )
    verdict = await asyncio.to_thread(gatekeeper.triage, event_text)
